...
<section id="widget-grid" class="">
    <div class="row">
        <article class="col-xs-12">
            <div class="jarviswidget jarviswidget-color-blue" id="wid-id-atendimento" data-widget-colorbutton="false" data-widget-editbutton="false" data-widget-deletebutton="false" data-widget-togglebutton="false">
                <header><span class="widget-icon"><i class="fa fa-list"></i></span><h2>Todos Atendimentos</h2></header>
                <div class="row" style="padding: 10px 15px 0;">
                    <form method="GET" action="{{ route('admin.atendimento.index') }}" class="smart-form client-form form_ajax">
                        <div class="row">
                ...
                <div class="table-responsive" style="padding:0 15px 15px;">
                    <p style="margin-top:10px;">@if($permissao>=2)<a class="btn btn-success btn-sm" href="{{ route('admin.atendimento.add') }}"><i class="fa fa-plus"></i> Novo</a>@endif</p>
                    <table class="table table-bordered table-striped">
                        <thead><tr><th>ID</th><th>Beneficiário</th><th>Tipo</th><th>CID</th><th>Data</th><th>Status</th><th class="actions">Ações</th></tr></thead>
                        <tbody>
                        @forelse($rows as $row)
                            <tr>
                                <td>{{ $row->id }}</td>
                                <td>{{ $row->beneficiario->nome ?? '-' }}</td>
                                <td>{{ ($tipoAtendimentoArr[$row->tipo_atendimento] ?? $row->tipo_atendimento) ?: '-' }}</td>
                                <td>{{ $row->cid ?? '-' }}</td>
                                <td>{{ optional($row->data_cadastro)->format('d/m/Y H:i') ?? '-' }}</td>
                                <td>{{ (int)$row->status===1?'Ativo':'Inativo' }}</td>
                                <td class="actions">
                                    <a href="{{ route('admin.atendimento.view',$row->id) }}" class="btn btn-xs btn-default"><i class="fa fa-eye"></i></a>
                                    @if($permissao>=2)<a href="{{ route('admin.atendimento.add',$row->id) }}" class="btn btn-xs btn-primary"><i class="fa fa-edit"></i></a>@endif
                                </td>
                            </tr>
