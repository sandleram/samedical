...
@section('title', 'Subfaturas')

@section('content')
<section id="widget-grid" class="">
    <div class="row">
        <article class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
            <div class="jarviswidget jarviswidget-color-blue" id="wid-id-subfatura"
                 data-widget-colorbutton="false" data-widget-editbutton="false"
                 data-widget-deletebutton="false" data-widget-togglebutton="false">
                <header>
                    <span class="widget-icon"><i class="fa fa-list"></i></span>
                    <h2>Todos Subfaturas</h2>
                </header>

                <div class="row" style="padding: 10px 15px 0;">
                    <form id="subfatura-search-form"
                          method="GET"
                          action="{{ route('admin.subfatura.index') }}"
                        ...
                        <div class="row">
                            ...
                            <section class="col col-2">
                                <label class="input">
                                    <input type="text" name="codigo" value="{{ $search['codigo'] ?? '' }}" placeholder="Codigo" class="input_login">
                                </label>
                            </section>
                            <section class="col col-1">
                                <label class="select">
                            ...
                            <section class="col col-2">
                                ...
                                @if ($hasFilter)
                                    <a href="{{ route('admin.subfatura.index') }}">Limpar Filtros</a>
                                @endif
                            </section>
                        </div>
                    </form>
                ...
                <div>
                    <div class="table-responsive" style="padding: 0 15px 15px;">
                        <p style="margin-top:10px;">
                            @if ($permissao >= 2)
                                <a class="btn btn-success btn-sm" href="{{ route('admin.subfatura.add') }}">
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                            @forelse ($subfaturas as $row)
                                <tr>
                                    <td>{{ $row->id }}</td>
                                    <td>{{ $row->codigo }}</td>
                                    <td>{{ $row->descricao }}</td>
                                    <td>{{ $row->beneficio->descricao ?? '-' }}</td>
                                    <td>{{ (int) $row->status === 1 ? 'Ativo' : 'Inativo' }}</td>
                                    <td class="actions">
                                        <a href="{{ route('admin.subfatura.view', $row->id) }}" class="btn btn-xs btn-default" title="Ver">
                                            <i class="fa fa-eye"></i>
                                        </a>
                                        @if ($permissao >= 2)
                                            <a href="{{ route('admin.subfatura.add', $row->id) }}" class="btn btn-xs btn-primary" title="Editar">
                        ...
                        <div style="padding:10px 0;">{{ $subfaturas->links() }}</div>
                    </div>
                </div>
            </div>
        </article>
