# Spec — mod-mh_critico (MH Crítico)
...
Portar o módulo legado `mh_critico` (`MhCriticoController`) para Laravel admin, preservando comportamento, ACL (`Modulo.controller` = `mh_critico`) e layouts SmartAdmin/Bootstrap 3.

Casos de MH crítico.

## Objetivo

Disponibilizar no `/admin` as mesmas telas e regras do Cake para **MH Crítico**, com middleware auth + tenant + permission alinhados ao AppController.
...
- Rotas `/admin/mh_critico/…` próximas ao legado
- ACL por action (níveis 1/2/3) com chave `mh_critico` (e chaves finas se houver)
...
Inventário em `legacy/app/View/MhCritico/`:

- `legacy/app/View/MhCritico/admin_index.ctp`
- `legacy/app/View/MhCritico/admin_view.ctp`
- `legacy/app/View/MhCritico/admin_add.ctp`
## Atores / perfis
...
- **Módulo / menu (`Modulo.controller`):** `mh_critico` ← chave ACL principal
...
1. Usuário autenticado acessa `/admin/mh_critico`
...
| Model | Tabela | Uso |
|-------|--------|-----|
| `MhCritico` | `mh_critico` | CRUD / consultas do módulo |
...
|--------|-----|-----------|
| `index` | `/admin/mh_critico` | Listagem |
| `show/view` | `/admin/mh_critico/view/{id}` | Detalhe |
| `add/edit` | `/admin/mh_critico/add[/id]` | Cadastro / edição |
...
- [ ] ACL com chave `mh_critico` e actions extras documentadas
...
- Controller legado: `legacy/app/Controller/MhCriticoController.php`
- Views legado: `legacy/app/View/MhCritico/`
- Plano-mestre: módulos por ondas (feature `mod-mh_critico`)
