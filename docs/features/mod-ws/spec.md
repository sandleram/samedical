...
Portar o módulo legado `ws` (`WsController`) para Laravel admin, preservando comportamento, ACL (`Modulo.controller` = `ws`) e layouts SmartAdmin/Bootstrap 3.

WS/BI Proativa; views não-admin em `legacy/app/View/Ws/`. Depende conexão `proativa`.

## Objetivo

Disponibilizar no `/admin` as mesmas telas e regras do Cake para **Web Service (WS)**, com middleware auth + tenant + permission alinhados ao AppController.
...
- Rotas `/admin/ws/…` próximas ao legado
- ACL por action (níveis 1/2/3) com chave `ws` (e chaves finas se houver)
...
Inventário em `legacy/app/View/Ws/`:

- `legacy/app/View/Ws/call_bi_beneficiarios.ctp` *(não-admin / WS)*
- `legacy/app/View/Ws/call_bi_beneficiarios2.ctp` *(não-admin / WS)*
...
- **Módulo / menu (`Modulo.controller`):** `ws` ← chave ACL principal
...
1. Usuário autenticado acessa `/admin/ws`
...
|--------|-----|-----------|
| WS BI | *(legado Ws)* | `call_bi_beneficiarios` / `call_bi_beneficiarios2` |
...
- [ ] ACL com chave `ws` e actions extras documentadas
...
- Controller legado: `legacy/app/Controller/WsController.php`
- Views legado: `legacy/app/View/Ws/`
- Plano-mestre: módulos por ondas (feature `mod-ws`)
