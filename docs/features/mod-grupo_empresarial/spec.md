# Spec — mod-grupo_empresarial (Grupo Empresarial)
...
Portar o módulo legado `grupo_empresarial` (`GrupoEmpresarialController`) para Laravel admin, preservando comportamento, ACL (`Modulo.controller` = `grupo_empresarial`) e layouts SmartAdmin/Bootstrap 3.

Cadastro de GE + tela crítica de seleção de contexto (tenant na sessão).

## Objetivo

Disponibilizar no `/admin` as mesmas telas e regras do Cake para **Grupo Empresarial**, com middleware auth + tenant + permission alinhados ao AppController.
...
- Rotas `/admin/grupo_empresarial/…` próximas ao legado
- ACL por action (níveis 1/2/3) com chave `grupo_empresarial` (e chaves finas se houver)
...
Inventário em `legacy/app/View/GrupoEmpresarial/`:

- `legacy/app/View/GrupoEmpresarial/admin_index.ctp`
- `legacy/app/View/GrupoEmpresarial/admin_view.ctp`
- `legacy/app/View/GrupoEmpresarial/admin_add.ctp`
- `legacy/app/View/GrupoEmpresarial/admin_selecione.ctp`

### Ações extras / ACL fina

- selecione — chave ACL fina `grupo_empresarial/selecione` (allow-always)
...
- **Módulo / menu (`Modulo.controller`):** `grupo_empresarial` ← chave ACL principal
...
1. Usuário autenticado acessa `/admin/grupo_empresarial`
...
| Model | Tabela | Uso |
|-------|--------|-----|
| `GrupoEmpresarial` | `grupo_empresarial` | CRUD / consultas do módulo |
...
|--------|-----|-----------|
| `index` | `/admin/grupo_empresarial` | Listagem |
| `show/view` | `/admin/grupo_empresarial/view/{id}` | Detalhe |
| `add/edit` | `/admin/grupo_empresarial/add[/id]` | Cadastro / edição |
| `selecione` | `/admin/grupo_empresarial/selecione` | Seleção de contexto tenant |
...
- [ ] ACL com chave `grupo_empresarial` e actions extras documentadas
...
- Controller legado: `legacy/app/Controller/GrupoEmpresarialController.php`
- Views legado: `legacy/app/View/GrupoEmpresarial/`
- Plano-mestre: módulos por ondas (feature `mod-grupo_empresarial`)
