# Spec — mod-atendimento (Atendimento)
...
Portar o módulo legado `atendimento` (`AtendimentoController`) para Laravel admin, preservando comportamento, ACL (`Modulo.controller` = `atendimento`) e layouts SmartAdmin/Bootstrap 3.

Registros de atendimento vinculados a beneficiário/cliente.

## Objetivo

Disponibilizar no `/admin` as mesmas telas e regras do Cake para **Atendimento**, com middleware auth + tenant + permission alinhados ao AppController.
...
- Rotas `/admin/atendimento/…` próximas ao legado
- ACL por action (níveis 1/2/3) com chave `atendimento` (e chaves finas se houver)
...
Inventário em `legacy/app/View/Atendimento/`:

- `legacy/app/View/Atendimento/admin_index.ctp`
- `legacy/app/View/Atendimento/admin_view.ctp`
- `legacy/app/View/Atendimento/admin_add.ctp`
## Atores / perfis
...
- **Módulo / menu (`Modulo.controller`):** `atendimento` ← chave ACL principal
...
1. Usuário autenticado acessa `/admin/atendimento`
...
| Model | Tabela | Uso |
|-------|--------|-----|
| `Atendimento` | `atendimento` | CRUD / consultas do módulo |
...
|--------|-----|-----------|
| `index` | `/admin/atendimento` | Listagem |
| `show/view` | `/admin/atendimento/view/{id}` | Detalhe |
| `add/edit` | `/admin/atendimento/add[/id]` | Cadastro / edição |
...
- [ ] ACL com chave `atendimento` e actions extras documentadas
...
- Controller legado: `legacy/app/Controller/AtendimentoController.php`
- Views legado: `legacy/app/View/Atendimento/`
- Plano-mestre: módulos por ondas (feature `mod-atendimento`)
