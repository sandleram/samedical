...
Portar o módulo legado `afastado` (`AfastadoController`) para Laravel admin, preservando comportamento, ACL (`Modulo.controller` = `afastado`) e layouts SmartAdmin/Bootstrap 3.

Gestão de afastamentos ocupacionais.

## Objetivo

Disponibilizar no `/admin` as mesmas telas e regras do Cake para **Afastado**, com middleware auth + tenant + permission alinhados ao AppController.
...
- Rotas `/admin/afastado/…` próximas ao legado
- ACL por action (níveis 1/2/3) com chave `afastado` (e chaves finas se houver)
...
Inventário em `legacy/app/View/Afastado/`:

- `legacy/app/View/Afastado/admin_index.ctp`
- `legacy/app/View/Afastado/admin_view.ctp`
- `legacy/app/View/Afastado/admin_add.ctp`
## Atores / perfis
...
- **Módulo / menu (`Modulo.controller`):** `afastado` ← chave ACL principal
...
1. Usuário autenticado acessa `/admin/afastado`
...
| Model | Tabela | Uso |
|-------|--------|-----|
| `Afastado` | `afastado` | CRUD / consultas do módulo |
...
|--------|-----|-----------|
| `index` | `/admin/afastado` | Listagem |
| `show/view` | `/admin/afastado/view/{id}` | Detalhe |
| `add/edit` | `/admin/afastado/add[/id]` | Cadastro / edição |
...
- [ ] ACL com chave `afastado` e actions extras documentadas
...
- Controller legado: `legacy/app/Controller/AfastadoController.php`
- Views legado: `legacy/app/View/Afastado/`
- Plano-mestre: módulos por ondas (feature `mod-afastado`)
