...
Portar o módulo legado `empresa` (`EmpresaController`) para Laravel admin, preservando comportamento, ACL (`Modulo.controller` = `empresa`) e layouts SmartAdmin/Bootstrap 3.

Empresas (CNPJ) sob cliente; usadas em beneficiário e faturamento.

## Objetivo

Disponibilizar no `/admin` as mesmas telas e regras do Cake para **Empresa**, com middleware auth + tenant + permission alinhados ao AppController.
...
- Rotas `/admin/empresa/…` próximas ao legado
- ACL por action (níveis 1/2/3) com chave `empresa` (e chaves finas se houver)
...
Inventário em `legacy/app/View/Empresa/`:

- `legacy/app/View/Empresa/admin_index.ctp`
- `legacy/app/View/Empresa/admin_view.ctp`
- `legacy/app/View/Empresa/admin_add.ctp`
## Atores / perfis
...
- **Módulo / menu (`Modulo.controller`):** `empresa` ← chave ACL principal
...
1. Usuário autenticado acessa `/admin/empresa`
...
| Model | Tabela | Uso |
|-------|--------|-----|
| `Empresa` | `empresa` | CRUD / consultas do módulo |
...
|--------|-----|-----------|
| `index` | `/admin/empresa` | Listagem |
| `show/view` | `/admin/empresa/view/{id}` | Detalhe |
| `add/edit` | `/admin/empresa/add[/id]` | Cadastro / edição |
...
- [ ] ACL com chave `empresa` e actions extras documentadas
...
- Controller legado: `legacy/app/Controller/EmpresaController.php`
- Views legado: `legacy/app/View/Empresa/`
- Plano-mestre: módulos por ondas (feature `mod-empresa`)
