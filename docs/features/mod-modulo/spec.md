# Spec — mod-modulo (Módulo (menu/ACL))
...
Portar o módulo legado `modulo` (`ModuloController`) para Laravel admin, preservando comportamento, ACL (`Modulo.controller` = `modulo`) e layouts SmartAdmin/Bootstrap 3.

Cadastro de módulos do menu; campo `controller` é a chave ACL (`Modulo.controller`).

## Objetivo

Disponibilizar no `/admin` as mesmas telas e regras do Cake para **Módulo (menu/ACL)**, com middleware auth + tenant + permission alinhados ao AppController.
...
- Rotas `/admin/modulo/…` próximas ao legado
- ACL por action (níveis 1/2/3) com chave `modulo` (e chaves finas se houver)
...
Inventário em `legacy/app/View/Modulo/`:

- `legacy/app/View/Modulo/admin_index.ctp`
- `legacy/app/View/Modulo/admin_view.ctp`
- `legacy/app/View/Modulo/admin_add.ctp`
## Atores / perfis
...
- **Módulo / menu (`Modulo.controller`):** `modulo` ← chave ACL principal
...
1. Usuário autenticado acessa `/admin/modulo`
...
| Model | Tabela | Uso |
|-------|--------|-----|
| `Modulo` | `modulo` | CRUD / consultas do módulo |
...
|--------|-----|-----------|
| `index` | `/admin/modulo` | Listagem |
| `show/view` | `/admin/modulo/view/{id}` | Detalhe |
| `add/edit` | `/admin/modulo/add[/id]` | Cadastro / edição |
...
- [ ] ACL com chave `modulo` e actions extras documentadas
...
- Controller legado: `legacy/app/Controller/ModuloController.php`
- Views legado: `legacy/app/View/Modulo/`
- Plano-mestre: módulos por ondas (feature `mod-modulo`)
