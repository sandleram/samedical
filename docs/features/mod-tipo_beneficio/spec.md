# Spec — mod-tipo_beneficio (Tipo de Benefício)
...
Portar o módulo legado `tipo_beneficio` (`TipoBeneficioController`) para Laravel admin, preservando comportamento, ACL (`Modulo.controller` = `tipo_beneficio`) e layouts SmartAdmin/Bootstrap 3.

Tipologia de benefícios (domínio auxiliar).

## Objetivo

Disponibilizar no `/admin` as mesmas telas e regras do Cake para **Tipo de Benefício**, com middleware auth + tenant + permission alinhados ao AppController.
...
- Rotas `/admin/tipo_beneficio/…` próximas ao legado
- ACL por action (níveis 1/2/3) com chave `tipo_beneficio` (e chaves finas se houver)
...
Inventário em `legacy/app/View/TipoBeneficio/`:

- `legacy/app/View/TipoBeneficio/admin_index.ctp`
- `legacy/app/View/TipoBeneficio/admin_view.ctp`
- `legacy/app/View/TipoBeneficio/admin_add.ctp`
## Atores / perfis
...
- **Módulo / menu (`Modulo.controller`):** `tipo_beneficio` ← chave ACL principal
...
1. Usuário autenticado acessa `/admin/tipo_beneficio`
...
| Model | Tabela | Uso |
|-------|--------|-----|
| `TipoBeneficio` | `tipo_beneficio` | CRUD / consultas do módulo |
...
|--------|-----|-----------|
| `index` | `/admin/tipo_beneficio` | Listagem |
| `show/view` | `/admin/tipo_beneficio/view/{id}` | Detalhe |
| `add/edit` | `/admin/tipo_beneficio/add[/id]` | Cadastro / edição |
...
- [ ] ACL com chave `tipo_beneficio` e actions extras documentadas
...
- Controller legado: `legacy/app/Controller/TipoBeneficioController.php`
- Views legado: `legacy/app/View/TipoBeneficio/`
- Plano-mestre: módulos por ondas (feature `mod-tipo_beneficio`)
