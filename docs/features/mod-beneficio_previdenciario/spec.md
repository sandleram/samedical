# Spec — mod-beneficio_previdenciario (Benefício Previdenciário)
...
Portar o módulo legado `beneficio_previdenciario` (`BeneficioPrevidenciarioController`) para Laravel admin, preservando comportamento, ACL (`Modulo.controller` = `beneficio_previdenciario`) e layouts SmartAdmin/Bootstrap 3.

Benefícios previdenciários.

## Objetivo

Disponibilizar no `/admin` as mesmas telas e regras do Cake para **Benefício Previdenciário**, com middleware auth + tenant + permission alinhados ao AppController.
...
- Rotas `/admin/beneficio_previdenciario/…` próximas ao legado
- ACL por action (níveis 1/2/3) com chave `beneficio_previdenciario` (e chaves finas se houver)
...
Inventário em `legacy/app/View/BeneficioPrevidenciario/`:

- `legacy/app/View/BeneficioPrevidenciario/admin_index.ctp`
- `legacy/app/View/BeneficioPrevidenciario/admin_view.ctp`
- `legacy/app/View/BeneficioPrevidenciario/admin_add.ctp`
## Atores / perfis
...
- **Módulo / menu (`Modulo.controller`):** `beneficio_previdenciario` ← chave ACL principal
...
1. Usuário autenticado acessa `/admin/beneficio_previdenciario`
...
| Model | Tabela | Uso |
|-------|--------|-----|
| `BeneficioPrevidenciario` | `beneficio_previdenciario` | CRUD / consultas do módulo |
...
|--------|-----|-----------|
| `index` | `/admin/beneficio_previdenciario` | Listagem |
| `show/view` | `/admin/beneficio_previdenciario/view/{id}` | Detalhe |
| `add/edit` | `/admin/beneficio_previdenciario/add[/id]` | Cadastro / edição |
...
- [ ] ACL com chave `beneficio_previdenciario` e actions extras documentadas
...
- Controller legado: `legacy/app/Controller/BeneficioPrevidenciarioController.php`
- Views legado: `legacy/app/View/BeneficioPrevidenciario/`
- Plano-mestre: módulos por ondas (feature `mod-beneficio_previdenciario`)
