# Spec — mod-perfil (Perfil)

> Stack: PHP 8.4+ · Laravel · MySQL 8 · Blade (SmartAdmin/Bootstrap 3) · Docker / nginx
>
> **Onda de implementação:** B · Grupo: Cadastros / ACL

## Contexto

Portar o módulo legado `perfil` (`PerfilController`) para Laravel admin, preservando comportamento, ACL (`Modulo.controller` = `perfil`) e layouts SmartAdmin/Bootstrap 3.

Perfis + permissões por módulo (`perfil_modulo.permissao` 0–3).

## Objetivo

Disponibilizar no `/admin` as mesmas telas e regras do Cake para **Perfil**, com middleware auth + tenant + permission alinhados ao AppController.
...
- Rotas `/admin/perfil/…` próximas ao legado
- ACL por action (níveis 1/2/3) com chave `perfil` (e chaves finas se houver)
...
Inventário em `legacy/app/View/Perfil/`:

- `legacy/app/View/Perfil/admin_index.ctp`
- `legacy/app/View/Perfil/admin_view.ctp`
- `legacy/app/View/Perfil/admin_add.ctp`

### Ações extras / ACL fina

- UI de perfil_modulo se existir no add/view
...
- **Módulo / menu (`Modulo.controller`):** `perfil` ← chave ACL principal
...
1. Usuário autenticado acessa `/admin/perfil`
...
| Model | Tabela | Uso |
|-------|--------|-----|
| `Perfil` | `perfil` | CRUD / consultas do módulo |
...
|--------|-----|-----------|
| `index` | `/admin/perfil` | Listagem |
| `show/view` | `/admin/perfil/view/{id}` | Detalhe |
| `add/edit` | `/admin/perfil/add[/id]` | Cadastro / edição |
...
- [ ] ACL com chave `perfil` e actions extras documentadas
...
- Controller legado: `legacy/app/Controller/PerfilController.php`
- Views legado: `legacy/app/View/Perfil/`
- Plano-mestre: módulos por ondas (feature `mod-perfil`)
