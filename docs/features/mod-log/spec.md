...
>
> **Onda de implementação:** B · Grupo: Cadastros / auditoria

## Contexto

Portar o módulo legado `log` (`LogController`) para Laravel admin, preservando comportamento, ACL (`Modulo.controller` = `log`) e layouts SmartAdmin/Bootstrap 3.

Consulta de logs (tipicamente somente leitura / index).

## Objetivo

Disponibilizar no `/admin` as mesmas telas e regras do Cake para **Log**, com middleware auth + tenant + permission alinhados ao AppController.
...
- Rotas `/admin/log/…` próximas ao legado
- ACL por action (níveis 1/2/3) com chave `log` (e chaves finas se houver)
...
Inventário em `legacy/app/View/Log/`:

- `legacy/app/View/Log/admin_index.ctp`
## Atores / perfis
...
- **Módulo / menu (`Modulo.controller`):** `log` ← chave ACL principal
...
1. Usuário autenticado acessa `/admin/log`
...
| Model | Tabela | Uso |
|-------|--------|-----|
| `Log` | `log` | CRUD / consultas do módulo |
...
- [ ] ACL com chave `log` e actions extras documentadas
...
- Controller legado: `legacy/app/Controller/LogController.php`
- Views legado: `legacy/app/View/Log/`
- Plano-mestre: módulos por ondas (feature `mod-log`)
