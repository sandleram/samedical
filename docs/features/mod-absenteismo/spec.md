# Spec — mod-absenteismo (Absenteísmo)
...
Portar o módulo legado `absenteismo` (`AbsenteismoController`) para Laravel admin, preservando comportamento, ACL (`Modulo.controller` = `absenteismo`) e layouts SmartAdmin/Bootstrap 3.

Absenteísmo; portar apenas CTPs ativos (não o backup).

## Objetivo

Disponibilizar no `/admin` as mesmas telas e regras do Cake para **Absenteísmo**, com middleware auth + tenant + permission alinhados ao AppController.
...
- Rotas `/admin/absenteismo/…` próximas ao legado
- ACL por action (níveis 1/2/3) com chave `absenteismo` (e chaves finas se houver)
...
Inventário em `legacy/app/View/Absenteismo/`:

- `legacy/app/View/Absenteismo/admin_index.ctp`
- `legacy/app/View/Absenteismo/admin_view.ctp`
- `legacy/app/View/Absenteismo/admin_add.ctp`

### Ações extras / ACL fina

- Ignorar `admin_add_bkp_full.ctp` (backup legado, não portar)
...
- **Módulo / menu (`Modulo.controller`):** `absenteismo` ← chave ACL principal
...
1. Usuário autenticado acessa `/admin/absenteismo`
...
| Model | Tabela | Uso |
|-------|--------|-----|
| `Absenteismo` | `absenteismo` | CRUD / consultas do módulo |
...
|--------|-----|-----------|
| `index` | `/admin/absenteismo` | Listagem |
| `show/view` | `/admin/absenteismo/view/{id}` | Detalhe |
| `add/edit` | `/admin/absenteismo/add[/id]` | Cadastro / edição |
...
- [ ] ACL com chave `absenteismo` e actions extras documentadas
...
- Controller legado: `legacy/app/Controller/AbsenteismoController.php`
- Views legado: `legacy/app/View/Absenteismo/`
- Plano-mestre: módulos por ondas (feature `mod-absenteismo`)
