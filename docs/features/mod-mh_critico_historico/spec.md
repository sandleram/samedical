# Spec — mod-mh_critico_historico (MH Crítico Histórico)
...
Portar o módulo legado `mh_critico_historico` (`MhCriticoHistoricoController`) para Laravel admin, preservando comportamento, ACL (`Modulo.controller` = `mh_critico_historico`) e layouts SmartAdmin/Bootstrap 3.

Histórico de eventos/status do MH crítico.

## Objetivo

Disponibilizar no `/admin` as mesmas telas e regras do Cake para **MH Crítico Histórico**, com middleware auth + tenant + permission alinhados ao AppController.
...
- Rotas `/admin/mh_critico_historico/…` próximas ao legado
- ACL por action (níveis 1/2/3) com chave `mh_critico_historico` (e chaves finas se houver)
...
Inventário em `legacy/app/View/MhCriticoHistorico/`:

- `legacy/app/View/MhCriticoHistorico/admin_index.ctp`
- `legacy/app/View/MhCriticoHistorico/admin_view.ctp`
- `legacy/app/View/MhCriticoHistorico/admin_add.ctp`
## Atores / perfis
...
- **Módulo / menu (`Modulo.controller`):** `mh_critico_historico` ← chave ACL principal
...
1. Usuário autenticado acessa `/admin/mh_critico_historico`
...
| Model | Tabela | Uso |
|-------|--------|-----|
| `MhCriticoHistorico` | `mh_critico_historico` | CRUD / consultas do módulo |
...
|--------|-----|-----------|
| `index` | `/admin/mh_critico_historico` | Listagem |
| `show/view` | `/admin/mh_critico_historico/view/{id}` | Detalhe |
| `add/edit` | `/admin/mh_critico_historico/add[/id]` | Cadastro / edição |
...
- [ ] ACL com chave `mh_critico_historico` e actions extras documentadas
...
- Controller legado: `legacy/app/Controller/MhCriticoHistoricoController.php`
- Views legado: `legacy/app/View/MhCriticoHistorico/`
- Plano-mestre: módulos por ondas (feature `mod-mh_critico_historico`)
