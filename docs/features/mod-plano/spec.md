...
Portar o módulo legado `plano` (`PlanoController`) para Laravel admin, preservando comportamento, ACL (`Modulo.controller` = `plano`) e layouts SmartAdmin/Bootstrap 3.

Planos vinculados a operadoras.

## Objetivo

Disponibilizar no `/admin` as mesmas telas e regras do Cake para **Plano**, com middleware auth + tenant + permission alinhados ao AppController.
...
- Rotas `/admin/plano/…` próximas ao legado
- ACL por action (níveis 1/2/3) com chave `plano` (e chaves finas se houver)
...
Inventário em `legacy/app/View/Plano/`:

- `legacy/app/View/Plano/admin_index.ctp`
- `legacy/app/View/Plano/admin_view.ctp`
- `legacy/app/View/Plano/admin_add.ctp`
## Atores / perfis
...
- **Módulo / menu (`Modulo.controller`):** `plano` ← chave ACL principal
...
1. Usuário autenticado acessa `/admin/plano`
...
| Model | Tabela | Uso |
|-------|--------|-----|
| `Plano` | `plano` | CRUD / consultas do módulo |
...
|--------|-----|-----------|
| `index` | `/admin/plano` | Listagem |
| `show/view` | `/admin/plano/view/{id}` | Detalhe |
| `add/edit` | `/admin/plano/add[/id]` | Cadastro / edição |
...
- [ ] ACL com chave `plano` e actions extras documentadas
...
- Controller legado: `legacy/app/Controller/PlanoController.php`
- Views legado: `legacy/app/View/Plano/`
- Plano-mestre: módulos por ondas (feature `mod-plano`)
