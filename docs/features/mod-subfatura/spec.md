...
Portar o módulo legado `subfatura` (`SubfaturaController`) para Laravel admin, preservando comportamento, ACL (`Modulo.controller` = `subfatura`) e layouts SmartAdmin/Bootstrap 3.

Subfaturas / rateio de faturamento.

## Objetivo

Disponibilizar no `/admin` as mesmas telas e regras do Cake para **Subfatura**, com middleware auth + tenant + permission alinhados ao AppController.
...
- Rotas `/admin/subfatura/…` próximas ao legado
- ACL por action (níveis 1/2/3) com chave `subfatura` (e chaves finas se houver)
...
Inventário em `legacy/app/View/Subfatura/`:

- `legacy/app/View/Subfatura/admin_index.ctp`
- `legacy/app/View/Subfatura/admin_view.ctp`
- `legacy/app/View/Subfatura/admin_add.ctp`
## Atores / perfis
...
- **Módulo / menu (`Modulo.controller`):** `subfatura` ← chave ACL principal
...
1. Usuário autenticado acessa `/admin/subfatura`
...
| Model | Tabela | Uso |
|-------|--------|-----|
| `Subfatura` | `subfatura` | CRUD / consultas do módulo |
...
|--------|-----|-----------|
| `index` | `/admin/subfatura` | Listagem |
| `show/view` | `/admin/subfatura/view/{id}` | Detalhe |
| `add/edit` | `/admin/subfatura/add[/id]` | Cadastro / edição |
...
- [ ] ACL com chave `subfatura` e actions extras documentadas
...
- Controller legado: `legacy/app/Controller/SubfaturaController.php`
- Views legado: `legacy/app/View/Subfatura/`
- Plano-mestre: módulos por ondas (feature `mod-subfatura`)
