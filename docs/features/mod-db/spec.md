# Spec — mod-db (DB (utilitário))

> Stack: PHP 8.4+ · Laravel · MySQL 8 · Blade (SmartAdmin/Bootstrap 3) · Docker / nginx
>
> **Onda de implementação:** F · Grupo: Integrações / utilitário

## Contexto

Portar o módulo legado `db` (`DbController`) para Laravel admin, preservando comportamento, ACL (`Modulo.controller` = `db`) e layouts SmartAdmin/Bootstrap 3.

Utilitário de banco (admin). Avaliar se ainda é necessário em Laravel (Artisan/migrations).

## Objetivo

Disponibilizar no `/admin` as mesmas telas e regras do Cake para **DB (utilitário)**, com middleware auth + tenant + permission alinhados ao AppController.
...
- Rotas `/admin/db/…` próximas ao legado
- ACL por action (níveis 1/2/3) com chave `db` (e chaves finas se houver)
...
Inventário em `legacy/app/View/Db/`:

- `legacy/app/View/Db/admin_index.ctp`
## Atores / perfis
...
- **Módulo / menu (`Modulo.controller`):** `db` ← chave ACL principal
...
1. Usuário autenticado acessa `/admin/db`
...
- [ ] ACL com chave `db` e actions extras documentadas
...
- Controller legado: `legacy/app/Controller/DbController.php`
- Views legado: `legacy/app/View/Db/`
- Plano-mestre: módulos por ondas (feature `mod-db`)
