...
```
routes/web.php
  → Interfaces\Http\Controllers\Admin\UsuarioController
    → Application\Usuario\*UseCase
      → Domain\Usuario\*RepositoryInterface
        → Infrastructure\EloquentUsuarioRepository
  → Blade admin/usuario/*
```
...
| Camada | Path |
|--------|------|
| Domain | `app/Domain/Usuario/` |
| Application | `app/Application/Usuario/` |
| Infrastructure | `app/Infrastructure/Persistence/Eloquent/EloquentUsuarioRepository.php` |
| Interfaces | `app/Interfaces/Http/Controllers/Admin/UsuarioController.php` + FormRequest |
| Shim | `app/Http/Controllers/Admin/UsuarioController.php` (deprecated) |
| Views | `resources/views/admin/usuario/*` (reutilizadas) |
