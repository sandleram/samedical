# Spec — mod-usuario (Usuário)

> Stack: PHP 8.4+ · Laravel · MySQL 8 · Blade (SmartAdmin/Bootstrap 3) · Docker / nginx
>
> **Onda de implementação:** B · Grupo: Cadastros / auth

## Contexto

Portar o módulo legado `usuario` (`UsuarioController`) para Laravel admin, preservando comportamento, ACL (`Modulo.controller` = `usuario`) e layouts SmartAdmin/Bootstrap 3.

CRUD de usuários admin + ações de sessão. Login/forgot ficam fora deste módulo (Auth).

## Objetivo

Disponibilizar no `/admin` as mesmas telas e regras do Cake para **Usuário**, com middleware auth + tenant + permission alinhados ao AppController.
...
- Rotas `/admin/usuario/…` próximas ao legado
- ACL por action (níveis 1/2/3) com chave `usuario` (e chaves finas se houver)
...
Inventário em `legacy/app/View/Usuario/`:

- `legacy/app/View/Usuario/admin_index.ctp`
- `legacy/app/View/Usuario/admin_view.ctp`
- `legacy/app/View/Usuario/admin_add.ctp`

### Ações extras / ACL fina

- atualiza_session_cliente (allow-always no AppController)
...
- **Módulo / menu (`Modulo.controller`):** `usuario` ← chave ACL principal
...
1. Usuário autenticado acessa `/admin/usuario`
...
| Model | Tabela | Uso |
|-------|--------|-----|
| `Usuario` | `usuario` | CRUD / consultas do módulo |
...
|--------|-----|-----------|
| `index` | `/admin/usuario` | Listagem |
| `show/view` | `/admin/usuario/view/{id}` | Detalhe |
| `add/edit` | `/admin/usuario/add[/id]` | Cadastro / edição |
...
- [ ] ACL com chave `usuario` e actions extras documentadas
...
- Controller legado: `legacy/app/Controller/UsuarioController.php`
- Views legado: `legacy/app/View/Usuario/`
- Plano-mestre: módulos por ondas (feature `mod-usuario`)
