# Spec — mod-home (Home / Dashboard)

> Stack: PHP 8.4+ · Laravel · MySQL 8 · Blade (SmartAdmin/Bootstrap 3) · Docker / nginx
>
> **Onda de implementação:** A · Grupo: Shell / dashboard

> Stub **rico** (Onda A): inventário completo de `.ctp` e gaps do piloto para o implementador.

## Contexto

Portar o módulo legado `home` (`HomeController`) para Laravel admin, preservando comportamento, ACL (`Modulo.controller` = `home`) e layouts SmartAdmin/Bootstrap 3.

Dashboard admin após login. Sempre liberado no AppController (`Home` em control_allowed).

## Objetivo

Disponibilizar no `/admin` as mesmas telas e regras do Cake para **Home / Dashboard**, com middleware auth + tenant + permission alinhados ao AppController.
...
- Rotas `/admin/home/…` próximas ao legado
- ACL por action (níveis 1/2/3) com chave `home` (e chaves finas se houver)
...
Inventário em `legacy/app/View/Home/`:

- `legacy/app/View/Home/admin_index.ctp`
...
- **Módulo / menu (`Modulo.controller`):** `home` ← chave ACL principal
...
1. Usuário autenticado acessa `/admin/home`
...
- [ ] ACL com chave `home` e actions extras documentadas
...
- Controller legado: `legacy/app/Controller/HomeController.php`
- Views legado: `legacy/app/View/Home/`
- Plano-mestre: módulos por ondas (feature `mod-home`)
