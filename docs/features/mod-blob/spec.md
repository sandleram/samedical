...
Portar o módulo legado `blob` (`BlobController`) para Laravel admin, preservando comportamento, ACL (`Modulo.controller` = `blob`) e layouts SmartAdmin/Bootstrap 3.

Download/armazenamento de arquivos em tabela blob; sem admin_*.ctp. Usado por vários módulos.

## Objetivo

Disponibilizar no `/admin` as mesmas telas e regras do Cake para **Blob (arquivos)**, com middleware auth + tenant + permission alinhados ao AppController.
...
- Rotas `/admin/blob/…` próximas ao legado
- ACL por action (níveis 1/2/3) com chave `blob` (e chaves finas se houver)
...
- download — allow-always `Blob/download` no AppController
...
- **Módulo / menu (`Modulo.controller`):** `blob` ← chave ACL principal
...
1. Usuário autenticado acessa `/admin/blob`
...
| Model | Tabela | Uso |
|-------|--------|-----|
| `Blob` | `blob` | CRUD / consultas do módulo |
...
- [x] Integração Blob download/upload

## Interface (admin)

| Action | URL | Descrição |
|--------|-----|-----------|
| `download` | `/admin/blob/download/{id}` | Download arquivo (allow-always) |
...
- [ ] ACL com chave `blob` e actions extras documentadas
...
- Controller legado: `legacy/app/Controller/BlobController.php`
- Views legado: `legacy/app/View/(sem View admin)/`
- Plano-mestre: módulos por ondas (feature `mod-blob`)
