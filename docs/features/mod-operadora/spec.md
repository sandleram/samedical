...
Portar o módulo legado `operadora` (`OperadoraController`) para Laravel admin, preservando comportamento, ACL (`Modulo.controller` = `operadora`) e layouts SmartAdmin/Bootstrap 3.

Cadastro de operadoras de saúde/plano.

## Objetivo

Disponibilizar no `/admin` as mesmas telas e regras do Cake para **Operadora**, com middleware auth + tenant + permission alinhados ao AppController.
...
- Rotas `/admin/operadora/…` próximas ao legado
- ACL por action (níveis 1/2/3) com chave `operadora` (e chaves finas se houver)
...
Inventário em `legacy/app/View/Operadora/`:

- `legacy/app/View/Operadora/admin_index.ctp`
- `legacy/app/View/Operadora/admin_view.ctp`
- `legacy/app/View/Operadora/admin_add.ctp`
## Atores / perfis
...
- **Módulo / menu (`Modulo.controller`):** `operadora` ← chave ACL principal
...
1. Usuário autenticado acessa `/admin/operadora`
...
| Model | Tabela | Uso |
|-------|--------|-----|
| `Operadora` | `operadora` | CRUD / consultas do módulo |
...
|--------|-----|-----------|
| `index` | `/admin/operadora` | Listagem |
| `show/view` | `/admin/operadora/view/{id}` | Detalhe |
| `add/edit` | `/admin/operadora/add[/id]` | Cadastro / edição |
...
- [ ] ACL com chave `operadora` e actions extras documentadas
...
- Controller legado: `legacy/app/Controller/OperadoraController.php`
- Views legado: `legacy/app/View/Operadora/`
- Plano-mestre: módulos por ondas (feature `mod-operadora`)
