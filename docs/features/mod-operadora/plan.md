...
```
routes/web.php
  → Interfaces\Http\Controllers\Admin\OperadoraController
    → Application\Operadora\*UseCase
      → Domain\Operadora\*RepositoryInterface
        → Infrastructure\EloquentOperadoraRepository
  → Blade admin/operadora/*
```
...
| Camada | Path |
|--------|------|
| Domain | `app/Domain/Operadora/` |
| Application | `app/Application/Operadora/` |
| Infrastructure | `app/Infrastructure/Persistence/Eloquent/EloquentOperadoraRepository.php` |
| Interfaces | `app/Interfaces/Http/Controllers/Admin/OperadoraController.php` + FormRequest |
| Shim | `app/Http/Controllers/Admin/OperadoraController.php` (deprecated) |
| Views | `resources/views/admin/operadora/*` (reutilizadas) |
