...
$(document).ready(function() {
	...
	if (!null) {
		...
		alert("Erro - Este menu não exite");
	}
	...
	$('#refresh').click(function(e) {
		$.SmartMessageBox({
			title : "<i class='fa fa-refresh' style='color:green'></i> Limpar Armazenamento Local",
			content : "Gostaria para redefinir todos os seus widgets salvos e LocalStorage claro?",
			buttons : '[Não][Sim]'
...
//			title : "<i class='fa fa-sign-out txt-color-orangeDark'></i> Deslogar <span class='txt-color-orangeDark'><strong>" + $('#show-shortcut').text() + "</strong></span> ?",
			title : "<i class='fa fa-sign-out txt-color-orangeDark'></i> Deslogar ?",
			content : $.logoutMSG || "Você pode melhorar sua segurança depois de sair fechando este navegador que está aberto!",
			buttons : '[Não][Sim]'
	...
	// LOCKUT BUTTON
	$('#lockout a').click(function(e) {
		//get the link
		var $this = $(this);
		$.loginURL = $this.attr('href');
		$.logoutMSG = $this.data('lockout-msg');

		// ask verification
		$.SmartMessageBox({
			title : "<i class='fa fa-sign-out txt-color-orangeDark'></i> Bloquear ?",
			content : $.logoutMSG || "Você perderá sua sessão mas o sistema lembrará seu usuário!",
			buttons : '[Não][Sim]'
                    ...
                    if (ButtonPressed == "Sim") {
                        $.root_.addClass('animated fadeOutUp');
                        //begin - redirecionamento automático
                        url = $('#lock_button').attr('rel');
                        window.location = $.loginURL+'?url='+encodeURIComponent(url);
                        
                        //end - redirecionamento automático
//                        $.ajax({
//                            url:"users/lockAjax",
//                            data: {'url':url},
//                            type: 'GET',
...
//                        });
        ...
        // LOCKUT BUTTON LOGIN /users/lock
        $('div.input-group-btn').click(function(e) {
                $.loginURL = '/users/lock';
		setTimeout(logout, 1000);
	});
        //FUNCTION CTRL
...
//                return true;
//            }
            if (event.which == 115) {//enter ou sim
                $('#bot2-Msg1').click();
                return true;
            }   
            if (event.which == 110) {//esc ou nao
                $('#bot1-Msg1').click();
                return true;
            } 
        });
	...
	if ($.fn.jarvisWidgets && $.enableJarvisWidgets) {
		...
		$('#widget-grid').jarvisWidgets({
			...
			customStart : function() {
				alert('Olá, este é um botão personalizado...')
			},
			customEnd : function() {
			...
			timestampFormat : 'Última Atualização: %m%/%d%/%y% %h%:%i%:%s%',
			refreshButton : true,
			refreshButtonClass : 'fa fa-refresh',
			labelError : 'Desculpe, houve um erro:',
			labelUpdated : 'Última atualização:',
			labelRefresh : 'Atualizar',
			labelDelete : 'Deletar widget:',
			afterLoad : function() {
			},
