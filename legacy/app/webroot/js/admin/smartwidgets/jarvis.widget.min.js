(function($, window, document, undefined) {
    ...
    Plugin.prototype = {_settings: function() {
            ...
            self.widget.each(function() {
                ...
                if (!thisHeader.parent().attr("role")) {
                    ...
                    if (self.o.deleteButton === true && tWidget.data("widget-deletebutton") === undefined) {
                        var deleteBtn = '<a href="javascript:void(0);" class="button-icon jarviswidget-delete-btn" rel="tooltip" title="Deletar" data-placement="bottom"><i class="' + self.o.deleteClass + '"></i></a>'
                    } else {
                        deleteBtn = ""
                    }
                    if (self.o.editButton === true && tWidget.data("widget-editbutton") === undefined) {
                        var editBtn = '<a href="javascript:void(0);" class="button-icon jarviswidget-edit-btn" rel="tooltip" title="Editar Título" data-placement="bottom"><i class="' + self.editClass[0] + '"></i></a>'
                    } else {
                        editBtn = ""
                    }
                    if (self.o.fullscreenButton === true && tWidget.data("widget-fullscreenbutton") === undefined) {
                        var fullscreenBtn = '<a href="javascript:void(0);" class="button-icon jarviswidget-fullscreen-btn" rel="tooltip" title="Tela Cheia" data-placement="bottom"><i class="' + self.fullscreenClass[0] + '"></i></a>'
                    } else {
                        fullscreenBtn = ""
                    }
                    ...
                    if (self.o.toggleButton === true && tWidget.data("widget-togglebutton") === undefined) {
                        ...
                        var toggleBtn = '<a href="#" class="button-icon jarviswidget-toggle-btn" rel="tooltip" title="Minimizar" data-placement="bottom"><i class="' + toggleSettings + '"></i></a>'
                    } else {
                        toggleBtn = ""
                    }
            ...
            self.widget.on(clickEvent, ".jarviswidget-delete-btn", function(e) {
                ...
                $.SmartMessageBox({title: "<i class='fa fa-times' style='color:#ed1c24'></i> " + self.o.labelDelete + ' "' + widTitle + '"', content: "Aviso: Tem certeza que deseja retirar esta janela?", buttons: "[Não][Sim]"}, function(ButtonPressed) {
                    var matchestit = widTitle.match(new RegExp("CV "));
                    var reltit = tWidget.children("header").attr("rel");
                    
                    if (ButtonPressed == "Sim") {
                        self._runLoaderWidget($(this));
                        $("#" + removeId).fadeOut(self.o.deleteSpeed, function() {
                            //BEGIN - REFAZENDO TELA PARA LIMPEZA DO CURRICULO POR EXCLUSÃO DO BOTÃO
                            if(matchestit != null){
                                $('.gerarCV').each(function() {
                                    if($(this).val() == reltit){
                                        $(this).removeAttr('checked');
                                    }
                                });
                                setTimeout(function() {
//                                    tWidget.parent().remove();
                                    $(this).remove();
                                }, 100);
                            }else{
                                $(this).remove();
                            }
                            //END - REFAZENDO TELA PARA LIMPEZA DO CURRICULO POR EXCLUSÃO DO BOTÃO
//                             $(this).remove();
