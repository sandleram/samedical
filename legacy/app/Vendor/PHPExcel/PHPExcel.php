...
class PHPExcel
{
    ...
    */
    ...
    public function _setCriador(){
        $this->getProperties()->setCreator("Sistemas Intranet Lapônia");
    }
    
    public function _setTitulo($titulo){
        $this->setTitle("Sistema de Movimento Diário de Caixa");
    }
    
    public function _setAssunto($assunto){
        $objPHPExcel->getProperties()->setSubject($assunto);
    }
    
    public function _setDescricao($descricao){
        $objPHPExcel->getProperties()->setDescription("Relatório de Movimentos de Caixa");
    }

    public function _setPalavrasChave($palavras){
        $objPHPExcel->getProperties()->setKeywords($palavras);
    }
    
    public function _ativaAba($num = 0, $titulo = false){
        if($titulo)
            $this->setActiveSheetIndex($num)->setTitle($titulo);
        else
            $this->setActiveSheetIndex($num);
    }
    
    public function _setVal($celula, $valor){
        $this->getActiveSheet()->setCellValue($celula, utf8_encode($valor));
    }
    
    public function _setFont($celula, $aFont){
        foreach($aFont AS $style => $val){
            switch($style){
                case 'bold': $this->getActiveSheet()->getStyle($celula)->getFont()->setBold(true); break;
                case 'size': $this->getActiveSheet()->getStyle($celula)->getFont()->setSize($val); break;
            }
        }
    }
    
    public function _mesclarCelula($intervalo_celula){
        $this->getActiveSheet()->mergeCells($intervalo_celula);
    }
    
    public function _setAutoSize($celula){
        $this->getActiveSheet()->getColumnDimension($celula)->setAutoSize(true);
    }
    
    
    public function _getConteudo(){
