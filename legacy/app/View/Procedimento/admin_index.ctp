...
<!--BEGIN - FILTRO -->
                    <div class="row">
                        <?php
                            ...
                            $s_id = $s_descricao = $s_tuss = $s_status = $s_operadora_id = $s_sinistro_evento_id = $s_tuss = '';
                            if(isset($search['id_'])){$s_id = $search['id_'];}
                            if(isset($search['descricao'])){$s_descricao = $search['descricao'];}
                            if(isset($search['sinistro_evento_id'])){$s_sinistro_evento_id = $search['sinistro_evento_id'];}
                            if(isset($search['operadora_id'])){$s_operadora_id = $search['operadora_id'];}
                            if(isset($search['tuss'])){$s_tuss = $search['tuss'];}
                        ...
                        ?>
                            <div class="row">
                                ...
                                <section class="col col-2">
                                    <label class="select">
                                        <?php echo $this->Form->input('sinistro_evento_id', array('label' => false, 'div' => false, 'placeholder' => 'Operadora', 'class' => 'input_login', 'options'=>$sinistroEventoArr, 'default' => '', 'value'=>$s_sinistro_evento_id)); ?>
                                        <i></i>
                                        <b class="tooltip tooltip-top-right"><i class="fa fa-chevron-right txt-color-blueLight"></i> Entre com o Status</b></label>
                                </section>
                                <section class="col col-2">
                                    <label class="select">
                                        <?php echo $this->Form->input('operadora_id', array('label' => false, 'div' => false, 'placeholder' => 'Operadora', 'class' => 'input_login', 'options'=>$operadoraArr, 'default' => '', 'value'=>$s_operadora_id)); ?>
                                        <i></i>
                                        <b class="tooltip tooltip-top-right"><i class="fa fa-chevron-right txt-color-blueLight"></i> Entre com o Status</b></label>
                                </section>
                                <section class="col col-2">
                                    <label class="input"> 
                                        <?php echo $this->Form->input('descricao', array('label' => false, 'div' => false, 'placeholder' => 'Descrição', 'class' => 'input_login','value'=>$s_descricao)); ?>
                                        <b class="tooltip tooltip-top-right"><i class="fa fa-chevron-right txt-color-blueLight"></i> Entre com a Descrição</b>
                                    </label>
                                </section> 
                                <section class="col col-2">
                                    <label class="input"> 
                                        <?php echo $this->Form->input('tuss', array('label' => false, 'div' => false, 'placeholder' => 'Tuss', 'class' => 'input_login','value'=>$s_tuss)); ?>
                                        <b class="tooltip tooltip-top-right"><i class="fa fa-chevron-right txt-color-blueLight"></i> Entre com o Código</b>
                                    </label>
                                </section> 
                                <section class="col col-2">
                                    <label class="select">
                                ...
                                <tbody>
                                    <?php foreach ($rows as $row): ?>
                                        <tr>
                                            ...
                                            <td><?php echo $row['SinistroEvento']['descricao'];?></td>
                                            <td> <?php 
                                                    if(isset($row['Operadora']['nome'])){
                                                        echo $row['Operadora']['nome'];
                                                    }else{
                                                        echo '(TUSS)';
                                                    }
                                                ?>
                                            </td>
                                            <td><?php echo $this->Funcoes->marcatexto($row[$TABLE]['tuss'],$s_tuss);?></td>
                                            <td><?php echo $this->Funcoes->marcatexto($row[$TABLE]['descricao'],$s_descricao);?></td>
                                    ...
                                    <?php endforeach; echo $this->Funcoes->semRegistro($rows,9,$search); ?>
                                </tbody>
                            </table>
                            <?php
                                /**
