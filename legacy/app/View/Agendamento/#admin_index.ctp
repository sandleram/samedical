            ...
            <div class="jarviswidget jarviswidget-color-blue" id="wid-id-1" data-widget-colorbutton="false" data-widget-editbutton="false"  data-widget-deletebutton="false" data-widget-togglebutton="false">
                ...
                <div class="row" style="display:;">
                    ...
                    <div class="row" >
                        ...
                        <?php if(in_array($this->Session->read('Auth.Usuario.perfil_id'), [1,2,3])){  ?>
                            ...
                            <tbody>
                                ...
                                foreach ($rows as $row) {
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <?php  echo $row['Atendimento']['Beneficiario']['nome']; ?>
                                            <?php if($row['Atendimento']['Beneficiario']['cpf']){?>
                                            <br><span class="note" style="font-size:10px;">(<?php echo $this->Funcoes->formata_cpf($row['Atendimento']['Beneficiario']['cpf']); ?>)</span>
                                            <?php }?>
                                        </td>
                                        <td ><?php 
                                        ...
                                        <td>
                                            <?php  
                                                if($row[$TABLE]['status'] == '1'){
                                                    echo '  <a href="'.Router::url('/admin/beneficiario/view/'.$row['Atendimento']['beneficiario_id'],true).'" class="btn btn-primary btn-xs " style="margin-top: 2px;">
                                                            <i class="fa fa-user"></i> Acessar
                                                        </a>';
                                                    echo '<a href="'.Router::url('/admin/agendamento/add/'.$row['Atendimento']['beneficiario_id'],true).'" class="btn btn-success btn-xs abrir_cria_programa " style="margin: 5px;">
                                                            <i class="fa fa-plus-circle"></i> Abrir Novo Agendamento
                                                        </a>';

                                                }else{
                                                    echo '  <a href="'.Router::url('/admin/atendimento/add/'.$row['Atendimento']['beneficiario_id'].'/'.$row[$TABLE]['atendimento_id'],true).'" class="ajaxMsg btn btn-success btn-xs abrir_cria_programa " style="margin: 2px;" ajaxmsg="Tem certeza que deseja Assumir o atendimento `'.$row['Atendimento']['Beneficiario']['nome'].'´ deste Beneficiário?">
                                                            <i class="fa fa-user-md"></i> Iniciar Atendimento
                                                        </a>';
                                                    echo '  <a href="'.Router::url('/admin/agendamento/cancelar_agendamento/'.$row['Atendimento']['beneficiario_id'].'/'.$row[$TABLE]['atendimento_id'],true).'" class="ajaxMsg btn btn-danger btn-xs " style="margin: 2px;" ajaxmsg="Tem certeza que deseja CANCELAR o programa `'.$row['Atendimento']['Beneficiario']['nome'].'´ deste Beneficiário?">
