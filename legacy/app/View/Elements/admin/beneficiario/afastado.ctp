                    ...
                    <?php 
                        if(count($row['listAfastado'])>0){
                            foreach($row['listAfastado'] as  $af){
                                ...
                                $situacao = '-';
                                $situacaoList = array(''=>' - ','A'=>'Afastado','RT'=>'Retorno ao Trabalho');
                                if(isset($situacaoList[$af['situacao']])){
                                    $situacao = $situacaoList[$af['situacao']];
                                }

                                echo "<tr>";
                                echo " <td>{$af['id']} ";
                                if($af['importacao_id'] != ''){
                                    echo "(<a href='".Router::url(array('controller'=>'importacao',@$af['importacao_id']) ,true)."'>#{$af['importacao_id']}</a>)";
                                }else{
                                    echo "(Manual)";
                                }                               
                                echo " </td>";
                                ...
                                echo " <td> {$this->Funcoes->dateToView(@$af['data_fim_afastamento'])} </td>";
                                echo " <td> {$af['cid']} </td>";
