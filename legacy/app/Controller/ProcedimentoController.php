...
class ProcedimentoController extends AppController {
    ...
    private $name_search;
    private $table;
    public $msg_nao_existe = 'Procedimento Inexistente';
    public $msg_salvo = 'A Procedimento foi SALVA com sucesso!';
    public $msg_salvo_erro = 'Não foi possível SALVAR a Procedimento, verifique as informações ou tente mais tarde! Caso o problem persista, informe o erro para o administrador do sistema.';
    public $msg_salvo_erro_login = 'Não foi possível efetuar o FILTRO ou SALVAR porque foi deslogado, tente novamente!';
    public $msg_excluido = 'A Procedimento foi EXCLUÍDA com sucesso!';
    public $msg_excluido_erro = 'Não foi possível EXCLUIR a Procedimento, tente mais tarde! Caso o problem persista, informe o erro para o administrador do sistema';
    ...
    public function beforeFilter() {
        ...
        $dataArr = $this->Beneficio->find('list',array('conditions'=>array('status < 2','cliente_id'=>$this->cliente_id),'fields'=>'id, descricao'));
        $dataDef = ($this->params['action'] == 'admin_index') ? 'Beneficio...' : 'Selecione...';
        $beneficioArr = $this->Funcoes->select_merge($dataArr, $dataDef);
        $this->set('beneficioArr', $beneficioArr);


        $this->loadModel('Operadora');
        $dataArr = $this->Operadora->find('list',array('conditions'=>array('status < 2'),'fields'=>'id,nome'));
        if($this->params['action'] == 'admin_index'){
            $dataArr['null'] = '(TUSS)';
        }
        $dataDef = ($this->params['action'] == 'admin_index') ? 'Operadora...' : 'Selecione...';
        $operadoraArr = $this->Funcoes->select_merge($dataArr, $dataDef);
        $this->set('operadoraArr', $operadoraArr);


        $this->loadModel('SinistroEvento');
        $dataArr = $this->SinistroEvento->find('list',array('conditions'=>array('status < 2'),'fields'=>'id,descricao'));
        $dataDef = ($this->params['action'] == 'admin_index') ? 'Sinistro Evento...' : 'Selecione...';
        $sinistroEventoArr = $this->Funcoes->select_merge($dataArr, $dataDef);
        $this->set('sinistroEventoArr', $sinistroEventoArr);
    ...
    public function admin_index() {
        ...
        if (is_array($search)):
            ...
            if (!empty($search['tuss'])):
                $buscaArr = explode(' ',$search['tuss']);
                if(count($buscaArr)> 0){
                   foreach($buscaArr as $vBusca){
                       $condition[] = $TABLE.'.tuss like "%' . $vBusca . '%"';
                   }
                }
            endif;
            if ($search['sinistro_evento_id']!= ''):
                if ($search['sinistro_evento_id'] == 'null'):
                    $condition[] = $TABLE.'.sinistro_evento_id is null ';
                else:
                    $condition[] = $TABLE.'.sinistro_evento_id = '. $search['sinistro_evento_id'];
                endif;
            endif;
            if ($search['operadora_id']!= ''):
                if ($search['operadora_id'] == 'null'):
                    $condition[] = $TABLE.'.operadora_id is null ';
                else:
...
//            throw new NotFoundException(__($this->msg_nao_existe));
    ...
    public function admin_add($id = null) {
        ...
        if ($this->request->is(array('post', 'put'))) {
            ...
            try {
                ...
                #ALTERAÇÃO DE PROCEDIMENTO PARA REPROCESSAMENTO DA IMPORTAÇÃO
                $seArr = array();
                if($id != null){
                    
                    #BUSCA PROCEDIMENTO ATUAL
                    $seArr = $this->$TABLE->find('first',array('conditions'=>array('id'=>$id),'fields'=>'sinistro_evento_id', 'recursive'=>-1));
                }
                ...
                #BUSCA PROCEDIMENTO ATUAL
                if(count($seArr)>0){
                    $sinistro_evento_id = $seArr[$TABLE]['sinistro_evento_id'];
                    if($sinistro_evento_id != $this->data[$TABLE]['sinistro_evento_id']){
                        #BUSCA SINISTROS QUE USARAM ESTE PROCEDIMENTO
                        $this->loadModel('Sinistro');
                        $importacao_ids = $this->Sinistro->find('list',array('conditions'=>array('procedimento_id'=>$id),'fields'=>'importacao_id,importacao_id'));

                        if(count($importacao_ids)>0){
                            $importacao_ids = implode(',',$importacao_ids);
                            $data_atual = date('Y-m-d H:i:s');
                            $sql = 'update importacao set status_processamento = 4, data_processamento = "'.$data_atual.'" where id in ('.$importacao_ids.')';
                ...
                $this->Log->create();
                $data_log = array('id' =>'',
                                  'log'                 => 'Erro Gravação - Procedimento',
                                  'mensagem'            => $flash,
                                  'description'         => json_encode($this->data),
    ...
    public function admin_busca_Procedimentos(){
        $data = array();
        $beneficioArr = $this->data['busca'];
        $beneficio_id = implode(',',$beneficioArr);
        $data = $this->Procedimento->find('list',array('conditions'=>array('status'=>1,'beneficio_id in ('.$beneficio_id.')'),'fields'=>'id,descricao'));
