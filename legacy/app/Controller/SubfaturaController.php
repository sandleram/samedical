...
class SubfaturaController extends AppController {
    
    #SUCESSO DEVE SER MÍNUSCULO
    public $components = array('Paginator','Funcoes');
    private $name_search;
    private $table;
    public $msg_nao_existe = 'Subfatura Inexistente';
    public $msg_salvo = 'A Subfatura foi SALVA com sucesso!';
    public $msg_salvo_erro = 'Não foi possível SALVAR a Subfatura, verifique as informações ou tente mais tarde! Caso o problem persista, informe o erro para o administrador do sistema.';
    public $msg_salvo_erro_login = 'Não foi possível efetuar o FILTRO ou SALVAR porque foi deslogado, tente novamente!';
    public $msg_excluido = 'A Subfatura foi EXCLUÍDA com sucesso!';
    public $msg_excluido_erro = 'Não foi possível EXCLUIR a Subfatura, tente mais tarde! Caso o problem persista, informe o erro para o administrador do sistema';
    ...
    public function beforeFilter() {
        ...
        $beneficioArr = $this->Beneficio->find('list',array('conditions'=>array('status < 2','cliente_id'=>$this->cliente_id),'fields'=>'id, descricao'));
        $benef0 = 'Selecione...';
        
        if($this->params['action'] == 'admin_index'){
            $benef0 = 'Beneficio...';
        }
        $beneficioArr = $this->Funcoes->select_merge($beneficioArr, $benef0);
    ...
    public function admin_index() {
        ...
        if (is_array($search)):
            ...
            if (!empty($search['codigo'])):
                $buscaArr = explode(' ',$search['codigo']);
                if(count($buscaArr)> 0){
                   foreach($buscaArr as $vBusca){
                       $condition[] = $TABLE.'.codigo like "%' . $vBusca . '%"';
                   }
                }
            endif;
            if ($search['beneficio_id']!= ''):
                $condition[] = $TABLE.'.beneficio_id = '. $search['beneficio_id'];
            endif;
            if ($search['status']!= ''):
        ...
        #BUSCA SOMENTE SUB DA EMPRESA
        $condition[] = 'Beneficio.cliente_id = '.$this->cliente_id;
...
//            throw new NotFoundException(__($this->msg_nao_existe));
    ...
    public function admin_busca_subfaturas(){
        $data = array();
        $beneficioArr = $this->data['busca'];
        $beneficio_id = implode(',',$beneficioArr);
        $data = $this->Subfatura->find('list',array('conditions'=>array('status'=>1,'beneficio_id in ('.$beneficio_id.')'),'fields'=>'id,descricao'));
