...
class BeneficioController extends AppController {

    public  $components = array('Paginator','Funcoes');
    private $name_search;
    private $table;
    public  $msg_nao_existe = 'Beneficio Inexistente';
    public  $msg_salvo = 'O Beneficio foi SALVO com sucesso!';
    public  $msg_salvo_erro = 'Não foi possível SALVAR o Beneficio, verifique as informações ou tente mais tarde! Caso o problem persista, informe o erro para o administrador do sistema.';
    public  $msg_excluido = 'O Beneficio foi EXCLUÍDO com sucesso ';
...
//            throw new NotFoundException(__($this->msg_nao_existe));
    ...
    public function  admin_busca_operadora_tipo_beneficio(){
    $data = array();
    $beneficioArr = @$this->data['busca'];
    $beneficioArr = array(1,2,3);
    $beneficios = implode(',',$beneficioArr);
    $data = $this->Beneficio->find('all',array('conditions'=>array('status'=>1,'id in ('.$beneficios.')'),'fields'=>'operadora_id,tipo_beneficio_id','recursive'=>-1));
    if(count($data)>0){
        $data = $this->Funcoes->retiraSubArray($data, 'Beneficio','all');
    }
    ...
    public function admin_verifica_breakeven(){
        $data = array();
        $beneficioArr = $this->data['busca'];
        $beneficio_id = implode(',',$beneficioArr);
        $data = $this->Beneficio->find('list',array('conditions'=>array('id in ('.$beneficio_id.')'),'fields'=>'id,breakeven'));
