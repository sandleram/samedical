...
class ImportacaoNovaController extends AppController
{
    ...
    public function admin_index($id = null)
    {
        ...
        $statusProcessoArr = $this->Funcoes->parametros('Status_Processo_Impo', 'list', NULL, true, 'Processo...');
        $this->set('statusProcessoArr', $statusProcessoArr);
...
#       https://github.com/PHPOffice/PHPExcel
    ...
    public function admin_add()
    {
        ...
        if ($this->request->is(array('post', 'put'))) {
            ...
            $FILE = $this->data[$TABLE]['arquivo'];
            $uploadFolder = WWW_ROOT . 'files' . DS . 'uploads' . DS . 'importacao_nova' . DS . 'aguardando' . DS;
            ...
            $fileNameArr = explode('.', $nome_arquivo);
            ...
            if ($successMove) {
                ...
                try {
                    ...
                    $dataImp['nome_arquivo'] = $nome_arquivo;
                    $dataImp['data_cadastro'] = $data_atual;
                    $dataImp['usuario_criador_id'] = $this->usuario_id;
                    $dataImp['status_processo'] = 0; #(0- pending, 1-processing, 2-done, 3-done_whitch_error, 4-error
                    $dataImp['status'] = 1;
                    ...
                    $dataSource->rollback();
                    $this->Session->setFlash("Erro na Importação !");
                    #GRAVA LOG
                    $this->loadModel('Log');
                    $this->Log->create();
                    $data_log = array(
                        'id' => '',
                        'log'                 => 'Erro - Importação Nova',
                        'description'         => json_encode($this->data),
                        'server_description'  => '',
                        'data_cadastro'       => date('Y-m-d H:i:s'),
                        'usuario_id'          => $this->Session->read('Auth.Usuario.id')
    ...
    {
        if (!$this->ImportacaoNova->exists($id)) {
            return $this->redirect(['action' => 'index']);
        }
        $row = $this->ImportacaoNova->findById($id);
        $this->set('row', $row);
    }
    ...
    {
        $this->autoRender = false;
        $import = $this->ImportacaoNova->findById($id);
        if (isset($import['ImportacaoNova']['status_processo'])) {
            echo  json_encode($import['ImportacaoNova']);
        } else {
            return;
        }
    }
    ...
    public function admin_processar_arquivo($id)
    {
        ...
        // 🔥 dispara o shell em background
        $isWindows = (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN');

        if ($isWindows) {

            $php  = 'C:\\wamp\\bin\\php\\php5.4.12\\php.exe';
            $cake = APP . 'Console\\cake.php';

            $cmd = sprintf(
                'cmd /c start "" /B "%s" "%s" import %d',
                $php,
                $cake,
                $id
            );
            ...
            $cmd = sprintf(
                'php %sapp/Console/cake.php import %d > /dev/null 2>&1 &',
                ROOT . DS,
                $id
            );
        }
        ...
        $this->Session->setFlash("Importação iniciada. Você pode sair da página.");
        return $this->redirect(['action' => 'admin_view', $id]);
        exit;
        #CONSULTA IMPORTACAO AGUARDANDO
        ...
        #DADOS COM ERRO
        #GERAR NOVO ARQUIVO COM DADOS ERRADOS 

        echo 'teste';
        // Disable CakePHP's rendering of views/layouts
        // $this->autoRender = false;
        ...
        if ($this->request->is(array('post', 'put'))) {
            ...
            $uploadFolder = WWW_ROOT . 'files' . DS . 'uploads' . DS . 'importacao_nova' . DS;
    ...
    public function send_file($id = null)
    {
        ...
        $dataArr = $this->ImportacaoNova->find('first', array('conditions' => array('id' => $id), 'fields' => 'arquivo_importado,status_processo', 'recursive' => -1));
        $caminho = [
            0 => 'aguardando',
            1 => 'processando',
            2 => 'concluido',
        ...
        //javascript:void(0);
        $url = 'files/uploads/importacao_nova/' . $caminho[$dataArr['ImportacaoNova']['status_processo']] . '/' . $dataArr['ImportacaoNova']['arquivo_importado'];
    ...
    /**
     * RETORNA O STATUS DA IMPORTAÇÃO VIA AJAX
     * 
     * 
        setInterval(() => {
            fetch('/importacoes/status/1')
                .then(r => r.json())
                .then(d => {
                    console.log(`${d.linhas_processadas}/${d.total_linhas}`);
                });
        }, 3000);
     */
}
