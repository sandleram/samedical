...
class ParametroController extends AppController {
    ...
    private $name_search;
    private $table;
    public  $msg_nao_existe = 'Parâmetro Inexistente';
    public  $msg_salvo = 'O Parâmetro foi SALVO com sucesso!';
    public  $msg_salvo_erro = 'Não foi possível SALVAR o Parâmetro, verifique as informações ou tente mais tarde! Caso o problem persista, informe o erro para o administrador do sistema.';
    public  $msg_excluido = 'O Parâmetro foi EXCLUÍDO com sucesso ';
    public  $msg_excluido_erro = 'Não foi possível EXCLUIR o Parâmetro, tente mais tarde! Caso o problem persista, informe o erro para o administrador do sistema';
    ...
    public function admin_index() {
        ...
        if (is_array($search)):
            ...
            if (!empty($search['tipo'])):
                $buscaArr = explode(' ',$search['tipo']);
                if(count($buscaArr)> 0){
                   foreach($buscaArr as $vBusca){
                       $condition[] = $TABLE.'.tipo like "%' . $vBusca . '%"';
                   }
                }
            endif;
            if (!empty($search['valor'])):
                $buscaArr = explode(' ',$search['valor']);
                if(count($buscaArr)> 0){
                   foreach($buscaArr as $vBusca){
                       $condition[] = $TABLE.'.valor like "%' . $vBusca . '%"';
                   }
                }
            endif;
...
//        debug($this->Parametro->getDataSource()->getLog(false, false));
        #CARREGA TODAS TIPOS
        $tipoArr = $this->Parametro->find('list',array('fields'=>'tipo,tipo', 'order'=>array('Parametro.tipo'), 'recursive'=>-1));
        $this->set(compact('tipoArr'));
        ...
        if ($this->request->is(array('post', 'put'))) {
            ...
            try {
                ...
                if(isset($this->data[$TABLE]['tipo_novo']) && $this->data[$TABLE]['tipo_novo'] != ''){
                    $this->request->data[$TABLE]['tipo'] = $this->data[$TABLE]['tipo_novo'];
                }
