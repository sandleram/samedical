...
class ImportacaoController extends AppController
{
    ...
    private $erro_linha = [];
    private $estrutura_banco = [];
    private $estrutura_banco_db = [];
    private $warning_error = [];
    private $warning_linha = [];
...
#       https://github.com/PHPOffice/PHPExcel
    ...
    public function admin_read_file_import($FILE, $limit = 200000)
    {
        ...
        $this->file_name = $this->Funcoes->normalizaeUrl($file_name) . '_' . time() . '.' . $extensao;
    

        #MONTA CAMINHO E NOME DO ARQUIVO E SALVA NO SERVIDOR
        $this->file_full = $uploadFolder . $this->file_name;


        if (move_uploaded_file($FILE['tmp_name'], $this->file_full)) {
            $data_save = [];
            $titulo = [];
            $row = 1;
            if (($handle = fopen($this->file_full, "r")) !== FALSE) {
    ...
    /**
     * BUSCA REGISTROS EXISTENTES NO BANCO DE DADOS
     * @param type $dataList
     * @return type
     */
    public function admin_search_list(){

        #BUSCA e VALIDA CPF e CNPJ EXISTENTES
        $listCpfs = [];
        $listCnpjs = [];
        foreach ($this->data_save as $data_k => $data_v) {
            $listCnpjs[] = (string) @$data_v['cnpj'];
            $listCpfs[] = (string) @$data_v['cpf'];
        }   

        #BUSCA EMPRESAS
        $empresasRs = $beneficiariosRsCliente = $beneficiariosRsOutros = [];
        $empresasRs = $this->Empresa->find('list', array(
            'conditions' => array(
                // 'Empresa.cliente_id' => $this->cliente_id,
                'Empresa.cnpj' => $listCnpjs
            ),
            'fields' => array('Empresa.cnpj', 'Empresa.id'),
            'recursive' => -1
        ));
        
        #BUSCA CPFS mesmo cliente e outro cliente
        $beneficiariosRs = $this->Beneficiario->find('all', array(
            'conditions' => array(
                'Beneficiario.cpf' => $listCpfs
            ),
            'fields' => array( 'Beneficiario.cpf', 'Beneficiario.id', 'Beneficiario.situacao', 'Beneficiario.cliente_id'),
            'recursive' => -1
        ));
        if(count($beneficiariosRs) >0){
            foreach ($beneficiariosRs as $b) {
                $cpf = $b['Beneficiario']['cpf'];
    
                $beneficiariosRsCliente[$cpf] = array(
                    'id'         => $b['Beneficiario']['id'],
                    'situacao'   => $b['Beneficiario']['situacao'],
                    'cliente_id' => $b['Beneficiario']['cliente_id']
                );
            }
        }


        #BUSCA AFASTADOS SE FOR DO TIPO DE IMPORTAÇÃO ATUAL
        $afastadoRsAfastado = [];
        if($this->tipo_importacao == 'afastado'){
            $this->loadModel('Afastado');
            $afastadoRsAfastadoAll = $this->Afastado->find('all', array(
                'joins' => array(
                    array(
                        'table' => 'beneficiario',
                        'alias' => 'Beneficiario',
                        'type' => 'INNER',
                        'conditions' => array(
                            'Afastado.beneficiario_id = Beneficiario.id'
                        )
                    )
                ),
                'conditions' => array(
                ...
                'fields' => array(
                    'Beneficiario.cpf',
                    'Afastado.id',
                    'Afastado.data_inicio_afastamento',
                    'Afastado.situacao'
                ),
                'recursive' => -1
            ));
            if(count($afastadoRsClienteAll) >0){
                foreach ($afastadoRsClienteAll as $a) {
                    $cpf = $a['Beneficiario']['cpf'];
        
                    $afastadoRsAfastado[$cpf] = array(
                        'id'                        => $a['Afastado']['id'],
                        'data_inicio_afastamento'   => $a['Afastado']['data_inicio_afastamento'],
                        'situacao'                  => $a['Afastado']['situacao']
                    );
                }
            }
        }
        ...
        $this->search_list = array(
            'empresas' => $empresasRs,
            'beneficiarios_cliente' => $beneficiariosRsCliente,
            'afastados' => $afastadoRsAfastado
        );
    ...
    public function admin_valida_dados_por_tipo(){
        
        #GERAL

        #BENEFICIARIO
        if ($this->tipo_importacao == 'beneficiario') {
            foreach($this->data_save as $data_k => $data_v){
                $this->erro_linha   = [];

                $this->data_save[$data_k]['cliente_id'] = (isset($data_v['cliente_id']) && $data_v['cliente_id'] != '') ? $data_v['cliente_id'] :  $this->Session->read('Auth.Usuario.cliente_id');
                ...
                $cpf = str_replace('-', '', $cpf);
                if ($cpf == '') {
                    $this->erro_linha[] = 'CPF é Obrigatório.';
                }else if (strlen($cpf) != 11) {
                    $this->erro_linha[] = 'CPF deve ser somente números e ter exatamente 11 números';
                }else{
                    if(isset($this->search_list['beneficiarios_cliente'][$cpf])){
                        if($this->data_save[$data_k]['cliente_id'] != $this->search_list['beneficiarios_cliente'][$cpf]['cliente_id']){
                            $this->erro_linha[] = 'Beneficiário já existe no banco de dados para outro cliente!';
                        }else{
                            if($this->data_save[$data_k]['situacao'] != $this->search_list['beneficiarios_cliente'][$cpf]['situacao']){
                                $this->data_save[$data_k]['beneficiario_id'] = $this->search_list['beneficiarios_cliente'][$cpf]['id'];
                            }else{
                                $this->erro_linha[] = 'Beneficiário existente com a mesma situação, não é possível alterar!';
                            }
                        }
                    }
                }
                $this->data_save[$data_k]['cpf'] = $cpf;
                ...
                $cnpj = str_replace('/', '', $cnpj);
                if ($cnpj == '') {
                    $this->erro_linha[] = 'CNPJ é Obrigatório';
                }else if (strlen($cnpj) != 14) {
                    $this->erro_linha[] = 'CNPJ deve ser somente números e ter exatamente 14 números';
                }else{
                    if(isset($this->search_list['empresas'][$cnpj])){
                        $this->data_save[$data_k]['empresa_id'] = $this->search_list['empresas'][$cnpj];
                    }
                }
                $this->data_save[$data_k]['cnpj'] = $cnpj;
                ...
                $this->data_save[$data_k]['nome'] = $nome;


                $data_nascimento = NULL;
                if ($data_v['data_nascimento'] != '') {
                    $data_nascimento = $this->Funcoes->dateToDb($data_v['data_nascimento']);
                    $this->valida_dado($data_nascimento, 'data_nascimento');
                }
                $this->data_save[$data_k]['data_nascimento'] = $data_nascimento;

                $this->valida_dado($data_v['sexo'], 'sexo');
        ...
        } else if ($this->tipo_importacao == 'afastado') {
            foreach($this->data_save as $data_k => $data_v){
                $this->erro_linha   = [];
                ...
                $this->data_save[$data_k]['nome_colaborador'] = $nome;
                ...
                if (!in_array($data_v['situacao'], ['A', 'RT'])) {
                    $this->erro_linha[] = 'SITUAÇÃO INVÁLIDA.';
                }
                ...
                $this->data_save[$data_k]['data_inicio_afastamento'] = $data_inicio_afastamento;

                $data_fim_afastamento = null;
                if ($data_v['data_fim_afastamento'] != '') {
                    $data_fim_afastamento = $this->valida_data($data_v['data_fim_afastamento'], 'Data Fim do Afastamento');
                }
                $this->data_save[$data_k]['data_fim_afastamento'] = $data_fim_afastamento;

                #antes o astamento id tinha outro validação com situacao, se fosse igual, pulava, agora não usa mais! 
                #$afastArr['Afastado']['situacao'] == $data_v['situacao'] => continue; pulava para o próximo

                $afastado_id = '';
                if(isset($this->search_list['afastados'][$cpf])){
                    if($this->data_save[$data_k]['data_inicio_afastamento'] != '' &&   
                        $this->data_save[$data_k]['data_inicio_afastamento'] == $this->search_list['afastados'][$cpf]['data_inicio_afastamento']){
                        $afastado_id = $this->search_list['afastados'][$cpf]['id'];
                    }
                }
                $this->data_save[$data_k]['afastado_id'] = $afastado_id;
        ...
        } else if ($this->tipo_importacao == 'beneficio_previdenciario') {
            foreach($this->data_save as $data_k => $data_v){
            }
        } else if ($this->tipo_importacao == 'absenteismo') {
            foreach($this->data_save as $data_k => $data_v){
            }
        }
    ...
    public function report_error_validate(){
    ...
    public function valida_tipo_importacao(){
        if(!in_array($this->tipo_importacao, [ 'beneficiario', 'afastado'])){
            $this->Session->setFlash('Aviso: Tipo de importação não disponível no momento, tente novamente mais tarde!');
    ...
    public function admin_add()
    {
        ...
        if ($this->request->is(array('post', 'put'))) {
            ...
            $this->tipo_importacao  = $this->data[$TABLE]['tipo_importacao'];
            $this->valida_tipo_importacao();
            
            $this->data_save = $this->admin_read_file_import($this->data['Importacao']['arquivo']);
            ...
            try {
                ...
                if (count($this->data_save) == 0 ) {
                    throw new Exception('Aviso: Não existem dados a serem gravados, verifique o arquivo e tente novamente!');
                }
              
                #VALIDA CABEÇALHO
                $this->valida_cabecalho($this->data_save[1]);
                
                #VALIDA QTD NA ÚLTIMA LINHA
                if (count($this->data_save[count($this->data_save)]) == 0) {
                    throw new Exception('Aviso: Existem 1 ou mais linhas vazias no final do arquivo, atualize e tente novamente!');
                }   
                
                #VALIDA CPF E CNPJ EXISTENTE
                $this->admin_search_list();

                #VALIDA DADOS POR TIPO DE IMPORTAÇÃO
                $this->admin_valida_dados_por_tipo();
                
                #DIRECIONA PARA TELA DE ERRO DE VALIDAÇÃO CASO EXISTA ERROS NO $this->erro
                $this->report_error_validate();
                ...
                $dataImp = [];
                $dataImp['id'] = '';
                $dataImp['tipo_importacao'] = $this->tipo_importacao;
                $dataImp['cliente_id'] = $this->Session->read('Auth.Usuario.cliente_id');
                $dataImp['arquivo_importado'] = $this->file_name;
                $dataImp['data_cadastro'] = date('Y-m-d H:i:s');
                $dataImp['usuario_criador_id'] = $this->Session->read('Auth.Usuario.id');
                $dataImp['status'] = 1;
                
                $this->Importacao->create();
                if (!$this->Importacao->save($dataImp)) {
                    throw new Exception('Erro: Não foi possível salvar a importação, tente novamente!');
                }

                $importacao_id = $this->Importacao->id;
                $dataImp['id'] = $importacao_id;
              
                $this->erro_exec = false;
                    
                #CARGA PÓS VALIDAÇÃO
                if ($this->tipo_importacao == 'beneficiario') {
                    $this->admin_carga_beneficiario($dataImp);
                } else if ($this->tipo_importacao == 'afastado') {
                    $this->admin_carga_afastado($dataImp);
                } else if ($this->tipo_importacao == 'beneficio_previdenciario') {
                    $this->admin_carga_beneficio_previdenciario( $dataImp);
                } else if ($this->tipo_importacao == 'absenteismo') {
                    $this->admin_carga_absenteismo( $dataImp);
                } else if ($this->tipo_importacao == 'sinistro') {
                    $this->admin_carga_sinistro( $dataImp);
                } else if ($this->tipo_importacao == 'fatura') {
                    $this->admin_carga_faturamento($dataImp);
                }

              
                if($this->erro_exec){
                    throw new Exception('Erro: Não foi possível salvar a importação, verifique os erros e tente novamente!');
                }

                #VERIFICAR DEPOIS QUANDO USUÁRIO EXISTENTE SE VAI SALVAR NOVA SITUAÇÃO PELA CHAMADA DO BENEFICIARIO
    ...
    public function admin_carga_beneficiario($dataImp)
    {
        $this->loadModel('Beneficiario');
        $this->loadModel('Empresa');
        ...
        foreach ($this->data_save as $data_k => $data_v) {
            ...
            try{
                if(isset($this->search_list['empresas'][$data_v['cnpj']])){
                    $empresa_id = $this->search_list['empresas'][$data_v['cnpj']];
                }else{
                    $this->Empresa->create();
                    $data_save_emp = [];
                    $data_save_emp['id'] = '';
                    $data_save_emp['cliente_id'] = $data_v['cliente_id'];
                    $data_save_emp['cnpj'] = $data_v['cnpj'];
                    $data_save_emp['razao_social'] = $data_v['razao_social'] ? $data_v['razao_social'] : 'Z - Criado via Importação!';
                    $data_save_emp['nome'] = $data_v['filial'] ? $data_v['filial'] : 'Z - Criado via Importação!';
                    ...
                    if (!$this->Empresa->save($data_save_emp)) {
                        ...
                        throw new Exception("Erro ao criar a empresa.");
                    }
                    
                    $empresa_id = $this->Empresa->id;
                    $this->search_list['empresas'][$data_v['cnpj']] = $empresa_id;
                }
            }catch(Exception $ex){
                ...
                // krumo( $data_save_emp);
                // debug($this->Empresa->getDataSource()->getLog(false, false));
                // exit;
                
                $this->erro_linha[] = $ex->getMessage();
                $this->erro_exec = true;
                break;
            }
            ...
            #CRIA BENEFICIARIO CASO NÃO EXISTA
            try{
                if(isset($data_v['beneficiario_id']) && $data_v['beneficiario_id'] != ''){
                    $dataB = [];
                    $dataB['id'] = $data_v['beneficiario_id'];
                    $dataB['empresa_id'] = $empresa_id;
                    $dataB['situacao'] = $data_v['situacao'];
                    $dataB['data_atualizacao'] = $dataImp['data_cadastro'];

                    if (!$this->Beneficiario->save($dataB)) {
                        $this->erro_linha[] = "Erro na atualização do beneficiario.";
                        throw new Exception("Erro ao criar a beneificiario.");
                    }
                }else{
                    $this->Beneficiario->create();
                    $dataS = [];
                    $dataS['cliente_id']        = $data_v['cliente_id'];;
                    $dataS['empresa_id']        = $empresa_id;
                    $dataS['nome_social']       = $data_v['nome_social'];
                    $dataS['nome']              = $data_v['nome'];
                    $dataS['cpf']               = $data_v['cpf'];
                    $dataS['data_nascimento']   = $data_v['data_nascimento'];
                    ...
                    $dataS['usuario_criador_id'] = $dataImp['usuario_criador_id'];
                    $dataS['usuario_atualizacao_id'] = NULL;
                    $dataS['data_atualizacao']  = NULL;
                    $dataS['status'] = 1;

                    if (!$this->Beneficiario->save($dataS)) {
                        $this->erro_linha[] = "Erro ao criar a beneificiario.";
                        throw new Exception("Erro ao criar a beneificiario.");
                    } 

                    $beneficiario_id = $this->Beneficiario->id;
                    $this->search_list['beneficiarios_cliente'][$data_v['cpf']] = ['id'=>$beneficiario_id,'situacao'=>$data_v['situacao'],'cliente_id'=>$data_v['cliente_id']];
                }
            }catch(Exception $ex){
                $this->erro_linha[] = $ex->getMessage();
                $this->erro_exec = true;
    ...
    public function admin_carga_afastado($dataImp)
    {
        $this->loadModel('Beneficiario');
        $this->loadModel('Empresa');
        $this->loadModel('Afastado');
        ...
        foreach ($this->data_save as $data_k => $data_v) {
            ...
            try{
                if(isset($data_v['beneficiario_id']) && $data_v['beneficiario_id'] != ''){
                    ...
                    $dataB['id'] = $data_v['beneficiario_id'];
                    $dataB['empresa_id'] = $empresa_id;
                    $dataB['situacao'] = 'Ativo';
                    $dataB['data_atualizacao'] = $dataImp['data_cadastro'];

                    if (!$this->Beneficiario->save($dataB)) {
                        $this->erro[] = array('linha' => $data_k, 'descricao' => 'ERRO CADASTRO DE BENEFICIARIO.');
                        $this->erro_exec = true;
                    }
                    $beneficiario_id = $data_v['beneficiario_id'];
                }else{
                    $this->Beneficiario->create();
                    $dataS = [];
                    $dataS['cliente_id']        = $data_v['cliente_id'];;
                    $dataS['empresa_id']        = $empresa_id;
                    $dataS['nome_social']       = (isset($data_v['nome_social']) ) ? $data_v['nome_social'] : null;#corrigir
                    $dataS['nome']              = $data_v['nome_colaborador'];
                    $dataS['cpf']               = $data_v['cpf'];
                    $dataS['pis']               = $data_v['pis'];
                    $dataS['data_nascimento']   = $data_v['data_nascimento'];
                    $dataS['nome_mae']          = $data_v['nome_mae'];
                    $dataS['nome_mae']          = $data_v['nome_mae'];
                    $dataS['ocupacao']          = $data_v['cargo'];
                    $dataS['endereco']          = $data_v['rua'] . ' ' . $data_v['num'] . ' ' . $data_v['complemento'];
                    $dataS['bairro']            = $data_v['bairro'];
                    $dataS['cidade']            = $data_v['cidade'];
                    $dataS['estado']            = $data_v['uf'];
                    $dataS['cep']               = $data_v['cep'];
                    $dataS['telefone']          = $data_v['telefone'];
                    $dataS['telefone1']         = $data_v['telefone1'];
                    $dataS['telefone2']         = $data_v['telefone2'];
                    $dataS['situacao']          = 'Ativo';
                    $dataS['importacao_id']     = $dataImp['id'];
                    $dataS['data_cadastro']     = $dataImp['data_cadastro'];
                    $dataS['usuario_criador_id'] = $dataImp['usuario_criador_id'];
                    $dataS['usuario_atualizacao_id'] = NULL;
            ...
            #AFASTADOS
            try{
                if(isset($data_v['afastado_id']) && $data_v['afastado_id'] != ''){
                    $dataA = [];
                    $dataA['id']                     = $data_v['afastado_id'];
                    $dataA['data_fim_afastamento']   = $data_v['data_fim_afastamento'];
                    $dataA['situacao']               = $data_v['situacao'];
                    $dataA['usuario_criador_id']     = $dataImp['usuario_criador_id'];
                    $dataA['usuario_atualizacao_id'] = $dataImp['data_cadastro'];
                    $dataA['status']                 = 1;

                    if (!$this->Afastado->save($dataA)) {
                        $this->erro[] = array('linha' => $data_k, 'descricao' => 'ERRO CADASTRO DE AFASTADO.');
                        $this->erro_exec = true;
                    } 
                }else{
                    $this->Afastado->create();
                    $dataA = [];
                    $dataA['importacao_id']             = $dataImp['id'];
                    $dataA['empresa_id']                = $empresa_id;
                    $dataA['data_inicio_afastamento']   = $data_v['data_inicio_afastamento'];
                    $dataA['beneficiario_id']           = $beneficiario_id;
                    $dataA['cid']                       = $data_v['cid'];
                    $dataA['tipo_afastamento']          = $data_v['tipo_afastamento'];
                    $dataA['assistencia_medica']        = $data_v['assistencia_medica'];
                    if (isset($data_v['plano_assistencia_medica'])) {
                        $dataA['plano_assistencia_medica']  = $data_v['plano_assistencia_medica'];
                    }
                    $dataA['data_fim_afastamento']      = $data_v['data_fim_afastamento'];
                    $dataA['situacao']                  = $data_v['situacao'];
                    $dataA['data_cadastro']             = $dataImp['data_cadastro'];
                    $dataA['usuario_criador_id']        = $this->usuario_id;
                    $dataA['status']                    = 1;

                    if (!$this->Afastado->save($dataA)) {
                        $this->erro_linha[] = "Erro ao criar a afastamento.";
                    } 

                    $Afastado_id = $this->Afastado->id;
                    $this->search_list['afastados'][$data_v['cpf']] = ['id'=>$Afastado_id,'data_inicio_afastamento'=>$dataA['data_inicio_afastamento'],'situacao'=>$dataA['situacao']];
                }
            }catch(Exception $ex){
                $this->erro_linha[] = "Erro ao criar a afastamento.";
            }
    ...
    public function valida_cabecalho($data)
    {
        ...
        if (count($estrutura_banco) != count($data)) {
            ...
            $this->Session->write('error_form', $this->data);
            $this->redirect(array('action' => 'add'));
            #END - RETORNO DAD  OS EM CASO DE ERRO NA INSERÇÃO
            exit;
        }
