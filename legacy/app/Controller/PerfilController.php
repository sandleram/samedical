...
class PerfilController extends AppController {
    ...
    private $name_search;
    private $table;
    public  $msg_nao_existe = 'Perfil Inexistente';
    public  $msg_salvo = 'O Perfil foi SALVO com sucesso!';
    public  $msg_salvo_erro = 'Não foi possível SALVAR o Perfil, verifique as informações ou tente mais tarde! Caso o problem persista, informe o erro para o administrador do sistema.';
    public  $msg_excluido = 'O Perfil foi EXCLUÍDO com sucesso ';
    public  $msg_excluido_erro = 'Não foi possível EXCLUIR o Perfil, tente mais tarde! Caso o problem persista, informe o erro para o administrador do sistema';
    ...
    public function beforeFilter() {
        ...
        $destaqueArr    = array('0' => 'Não', '1' => 'Sim'); 
        $this->set(compact('destaqueArr'));

        $this->set('tipo',[''=>'Selecione...','0'=>'Operacional','1'=>'Administrativo','2'=>'Master']);
...
//            throw new NotFoundException(__($this->msg_nao_existe));
        ...
        $modulos = $this->Modulo->find('all',array('conditions'=>array('status'=>1),'recursive'=>-1, 'order'=>'order'));
        ...
        if ($this->request->is(array('post', 'put'))) {
            ...
            try {
                ...
                if (empty($this->data[$TABLE]['id'])) {
                    ...
                    $this->request->data[$TABLE]['usuario_criador'] = $user_create;
                }else{
                ...
                #BEGIN - PERMISSOES USERS_X_MODULOS
                $this->loadModel('PerfilModulo');
                if (count($this->data['PerfilModulo']) > 0) {
                    #krumo($this->data);
                    foreach ($this->data['PerfilModulo'] as $kUM => $vUM):
                        $dataUM = array();
                        $dataUM['id'] = (isset($vUM['id']))? $vUM['id'] : '';
                        $dataUM['modulo_id'] = $kUM;
                        $dataUM['permissao'] = $vUM['permissao'];
                        $dataUM['perfil_id'] = $id;

                        
                        
                        if (isset($dataUM['id']) && $dataUM['id'] != '') {
                            if (!$this->PerfilModulo->exists($dataUM['id'])) {
                                throw new Exception();
                            }
                            $dataUM['data_atualizacao'] = $dateTimeNow;
                        } else {
                            $this->PerfilModulo->create();
                            $dataUM['data_cadastro'] = $dateTimeNow;
                        }
                        
                        if (!$this->PerfilModulo->save($dataUM)) {
                            throw new Exception();
                        }
                    endforeach;
                }
               
                #END - PERMISSOES USERS_X_MODULOS
